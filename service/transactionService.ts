...
import { getLast12Months, getLast7Days, getYearRange } from "../utils/helpers";
...
const updateWalletForNewTransaction = async (
  ...
  try {
    ...
    if (type === "expense" && walletData.amount! - amount < 0) {
      return { success: false, msg: "Wallet does not have enough balance" };
    }
    ...
    console.log(`Error in updating wallet for new transaction: ${err}`);
  ...
  try {
    ...
    if (image) {
      ...
      if (!imageUploadRes.success) {
        return {
          success: false,
          msg: imageUploadRes.msg || "Failed to upload attachment",
        };
      }
...
export const fetchWeeklyStats = async (uid: string): Promise<ResponseType> => {
  try {
    ...
    const stats = weekData.flatMap((day) => [
      {
        value: day.income,
        label: day.day,
        spacing: scale(4),
